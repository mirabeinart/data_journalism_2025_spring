...
**A1.** https://datawrapper.dwcdn.net/HTd0W/1/ The 2020 data only has 96% of votes? Why? Is it because a lot of people just didn't vote during the pandemic? But I'm sure a lot of people also didn't vote in the other elections? Was it an error in the data? Also, in-person voting on Election Day went nearly right back to the same amount as before the pandemic, which I was surprised by. 
...
#make the data into a format that i want for the graph. chat helped me with this and i didn't think it was going to work and then it did!
idk <- md_votes_by_type |>
  select(year, early_pct, mail_pct, election_pct) |>
  pivot_longer(cols = -year, names_to = "Voting_Type", values_to = "Percent")

write_csv(idk, "data/idkman.csv")
...
**A2.** https://datawrapper.dwcdn.net/7pxUR/1/ I compared the turnout in early votes between males and females in PG County. Females did a lot more early voting than men, which I wasn't surprised about (tbh, go women).

```{r}
men <- read_csv("data/ev_totals_men.csv") |>
  clean_names()

women <- read_csv("data/ev_totals_women.csv") |>
  clean_names()

#adding m and f to the column names so that i can join the two datasets and compare early voting between men and women in certain counties. chat helped me with this
colnames(men)[-1] <- paste0(colnames(men)[-1], "_m")
colnames(women)[-1] <- paste0(colnames(women)[-1], "_f")

combined_vote_things <- left_join(men, women, by ="day") |>
  select(day, prince_georges_m, prince_georges_f)


write_csv(combined_vote_things, "data/combined_vote_things.csv")
```
