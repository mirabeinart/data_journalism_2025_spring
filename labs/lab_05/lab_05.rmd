...
july_contributions <- read_csv("data/july_harris.csv")
august_contributions <- read_csv("data/august_harris.csv")
summer_contributions <- bind_rows(list(july_contributions, august_contributions))
```
...
harris_new_voters <- summer_contributions |>
...
A1. I joined the name, address, and city columns because they are the ones that are most likely to be similar between the two datasets. 
...
A2. 326 donations have come from people who registered to vote this year. 283 unique people. Brandyn Best has given the highest number of donations (5) since July 20.

```{r}
harris_new_voters |>
  filter(CountyRegistrationDate >= mdy("07/20/24")) |>
...
A3. 160 female Democrats, 123 male Democrats, and not that many Republicans. 32 total donations from non-Democrats.
