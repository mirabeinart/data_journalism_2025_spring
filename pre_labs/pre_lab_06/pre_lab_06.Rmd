...
**Task** Export the CSV file to your pre_lab_06 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** The instructions on the sheet (US Senator, Vote for 1) are included as columns, but have no data (which makes sense, because those weren't candidate options, obviously).
...
**Task** Get rid of rows we don't need by running the codeblock below. What problems remain? **Answer** The column names (after candidate) are not named correctly - they represent something (not just X2, X3, etc).
...
**Task** Run the codeblock below. What changed, and what issues or problems remain? **Answer** The code added a new column for office (House or Senate), but some of the parties are still labeled incorrectly (R/rWeldonED, RLake ED, etc)
