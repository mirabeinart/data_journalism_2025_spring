...
**Task** Load the five CSV files. Do they have the same column structure? **Answer** The data from 2020 and 2022 has "OTH" before "UNA", and is flipped in 2016, 2018, and 2024.
...
glimpse(county_voters_combined)
```
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** There should be 24 rows, and there are! Yay
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **Answer** The output message reads "Joining with `by = join_by(COUNTY)`"
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** PG County has the highest percentage of 18+ Black population as well as Democrats.
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** Howard County has the highest percentage of 18+ Asian population, and PG County still has the highest percentage of Democrats.
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **Answer** The ZCTA5N column is dbl datatype. 
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **Answer** It is now a chr datatype!
