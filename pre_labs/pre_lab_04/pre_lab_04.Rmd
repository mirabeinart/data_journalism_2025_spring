...
**Task** Run the following code to group and count records in the recipient_name column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** The first 6 rows of data are presumeably all towards the same recipient, but since they are formatted differently (some have periods after CO, different capitalization, wording), they are grouping into separate categories. 
...
**Task** Run the following code to group by recipient_clean and recipient_name, then filter for cities that start with W. Name three changes the refinr function made in the first 20 results. **Answer** It standardized W. Millar & Co to 'W. MILLAR & CO.' and the name with catering to '	W. MILLAR & CO. CATERING.' It also standardized Walmart to all caps and any variation of the Washington Hilton to '	WASHINGTON HILTON.'
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by city_clean and city to examine cities that start with A. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** I don't think this is the right question?? But I also don't see much of a difference between this data and when we ran a different refinr function earlier. 
...
```{r}
dc_food_cleaned <- read_csv("data/dc-food-cleaned.csv")
```
```{r}
dc_food_cleaned |>
