...
**Answer** 131,395 observations, 24 variables
...
In the space below, write the data type for the "last_name" column and the "amount" columns (it's in light gray, offset by \< \>). Also offer your best guess as to what the "last_name" column and the "amount" column represent. **Answer** "last_name" = <chr>, "amount" = <dbl>
...
In the space below, write the first and last name from the *second* row of the data. **Answer** Peggy Mukai
...
Describe what you think this code is doing below. What does the answer represent, compared to the results from `head()`? The summarize function aggregates the data and finds the average of the data, whereas the head function just shows you the top 6 rows of data. I think the data from summarize is calcuating the average of count_contribs, whatever that is, per date. 
...
**Answer** It arranged the data in descending order of count_contribs.
...
**Answer** It changed what the data was being grouped by -- it went from date to memo text and donation amount. It gave us more information because it showed us where the donations were going to, rather than just how many contributions were made on a certain date. 
...
**Answer** This codeblock grouped the data by where the donation was going and provided the total number of contributions, as well as the total amount of money donated to each committee/fundraising place. 
...
**Answer** This codeblock provided us with the median and mean of the donations for each fundraising committee. The Trump National Committee JFC was the top fundraising committee, followed by the Trump Save American Joint Fundraising Committee and NRSC. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? How is the data sorted? **Answer** This codeblock showed us where each donation was coming from -- cities in Maryland -- and is sorted by which city donated the most money towards republican campaigns. Annapolis was first (can't wait to move there!), followed by Bethesda, Baltimore, and Potomac.
