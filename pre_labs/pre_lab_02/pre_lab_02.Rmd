...
**Answer** The code is creating a new column for what percent of votes were cast on election day, rather than early voting or mail-in ballots by taking values from 2 existing columns (election_day and votes) and calculating the difference between the two. 
...
**Answer** The results are different from the previous task because they are now ordered from highest percentage of votes on election day (100%) to the lowest (0%).
...
**Answer** This answer shows us which candidates had the least amount of election day votes compared to total votes, which can show us which candidates relied more heavily on early voting and mail-in ballots. 
...
**Answer** str_to_lower() converts a character column into lowercase, and str_to_title() converts to title case, where the first letter of each word is capitalized (https://stringr.tidyverse.org/reference/case.html)
...
**Answer** By using mutate(), we are creating a new column, amount_category, based on the "amount" column, which reflects the monetary amount of each donation. Then, using case_when(), we are creating conditions for each amount_category based on the number in the amount column -- if it's less than 100, it gets tagged "Less than $100," if it's greater than or equal to 100 but less than 500, it gets tagged at "Between 100 and 499," and so on. 
...
**Answer** I think it's important because it doesn't change the original dataset with whatever analysis we might want to do.
...
**Answer:** The code changed the expentidure_date column into date format rather than character.
...
**Answer** The values in the month column groups all expenditures from the same month into one column, adding them all together. This is showing trends in spending based on month, with June being the highest.
